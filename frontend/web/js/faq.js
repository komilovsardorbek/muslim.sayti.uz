jQuery(document).ready(function ($) {
    ...
    if (articlesWrapper.length > 0) {
        var windowHeight = $(window).height(), articles = articlesWrapper.find('article'), aside = $('.cd-read-more'),
            articleSidebarLinks = aside.find('li');
        var scrolling = false, sidebarAnimation = false, resizing = false, mq = checkMQ(),
        ...
        aside.on('click', 'a', function (event) {
            ...
            $('body,html').animate({'scrollTop': selectedArticleTop + 2}, 300, function () {
                checkRead();
                $(window).on('scroll', checkRead);
            });
        });
    ...
    function updateArticle() {
        ...
        articles.each(function () {
            var article = $(this), articleTop = article.offset().top, articleHeight = article.outerHeight(),
    ...
    function updateSidebarPosition() {
        var articlesWrapperTop = articlesWrapper.offset().top, articlesWrapperHeight = articlesWrapper.outerHeight(),
...
$(document).ready(function () {
    $('.faq_inner1').click(function () {
        if ($(this).next(".faq_inner").hasClass('active')) {
            $(this).next(".faq_inner").removeClass('active').slideUp()
        } else {
            $(".faq_inner").removeClass('active').slideUp()
            $(this).next('.faq_inner').addClass('active').slideDown();
        }
        $(this).find('.faq_icon').toggleClass('rotate');
    });
});
...
function FaqFunction() {
    document.getElementById('sub_menu-tup').style.display = "block";
}

// Api geolocation uzbekisran
